...
type WebColor = keyof typeof COLORS;
...
type ColorBrightnessInfo = {
	brightness: number,
	isGood: boolean,
};

type ColorDifferenceInfo = {
	difference: number,
	isGood: boolean,
};
...
export function getColorInfo(color: WebColor) {
	return COLORS[color];
}


export function getColorRGB(color: WebColor | string): RGBArray {
 ...
 * @returns {ColorBrightnessInfo} Contrast value. If value is higher than 125 then
 * brightness is good.
*/
export function calcColorBrightness(r: number, g: number, b: number): ColorBrightnessInfo {
 ...
 * @see https://www.w3.org/TR/AERT/#color-contrast
*/
export function calcColorDifference(
	foregroundColor: WebColor | string,
	backgroundColor: WebColor | string
): ColorDifferenceInfo {
...
*/
export function calcAverageColorFromImageData(
	imageData: ImageData,
	backgroundColor: WebColor | string
) {
